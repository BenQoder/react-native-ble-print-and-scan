...
class HybridBleScanner: HybridBleScannerSpec() {
    ...
    override fun getScannerInfo(deviceId: String): Promise<ScannerInfo> {
        return Promise.async {
            ...
            val firmwareVersion = try {
                scannerManager.sendCommand(deviceId, ScannerBluetoothManager.READ_FIRMWARE_VERSION)
            } catch (e: Exception) {
                "Unknown"
            }
            ...
            ScannerInfo(
                ...
                batteryLevel = null, // Could be retrieved with battery command
                isConnected = scannerManager.isConnected(deviceId)
            )
        }
    }
    
    override fun getScannerSettings(deviceId: String): Promise<ScannerCurrentSettings> {
        return Promise.async {
            if (!scannerManager.isConnected(deviceId)) {
                throw Exception("Scanner $deviceId is not connected")
            }
            ...
            ScannerCurrentSettings(
                workMode = ScannerMode.HOST_TRIGGER,
